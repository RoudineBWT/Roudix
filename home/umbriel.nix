...
let
  ...
  # Génération de la config TOML en tant que fichier séparé
  umbrielConfig = pkgs.writeText "umbriel-config.toml" ''
...
in
{
  config = lib.mkIf isUmbriel {
    # ── Umbriel (module officiel - sans settings) ──────────────────────
    programs.umbriel = {
      enable = true;
      package = inputs.umbriel.packages.${pkgs.stdenv.hostPlatform.system}.default;
      # Ne pas utiliser settings pour éviter l'erreur
    };
    ...
    # On copie le fichier généré manuellement
