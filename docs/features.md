...
- OBS Studio, individually toggleable via `roudix.contentCreation.obs.enable` (default `true`), plugins à la carte via `roudix.contentCreation.obs.plugins` (`vkcapture` + `pipewire-audio-capture` by default; also available: `background-removal`, `move-transition`, `multi-rtmp`, `gstreamer`, `composite-blur`, `advanced-scene-switcher`, `input-overlay`, `waveform`)
